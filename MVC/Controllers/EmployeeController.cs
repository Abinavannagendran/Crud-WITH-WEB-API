...
namespace MVC.Controllers
{
    public class EmployeeController : Controller
    {
        ...
        {
            IEnumerable<mvcEmployeemodels> empList;
            HttpResponseMessage response = GlobalVariables.WebApiClient.GetAsync("Employee").Result;
            empList = response.Content.ReadAsAsync<IEnumerable<mvcEmployeemodels>>().Result;
            return View(empList);
        }
        ...
        public ActionResult AddOrEdit(int id = 0)
        {
            ...
            if (id == 0)
                return View(new mvcEmployeemodels());
            else
            {
                HttpResponseMessage response = GlobalVariables.WebApiClient.GetAsync("Employee/" + id.ToString()).Result;
                return View(response.Content.ReadAsAsync<mvcEmployeemodels>().Result);
            }

        }
        [HttpPost]
        public ActionResult AddOrEdit(mvcEmployeemodels emp)
        {
            if (emp.EmployeeID == 0)
            {
